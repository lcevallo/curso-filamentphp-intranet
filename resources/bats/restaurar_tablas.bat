...
"%PGBIN%\psql" -U %PGUSER% -d %PGDATABASE% -c "DROP TABLE IF EXISTS countries CASCADE;"
"%PGBIN%\psql" -U %PGUSER% -d %PGDATABASE% -c "DROP TABLE IF EXISTS states CASCADE;"
"%PGBIN%\psql" -U %PGUSER% -d %PGDATABASE% -c "DROP TABLE IF EXISTS cities CASCADE;"

@REM Restaurar las tablas desde el archivo de respaldo
echo Restaurando las tablas desde el respaldo...
"%PGBIN%\psql" -U %PGUSER% -d %PGDATABASE% -1 -f backup_countries_states_cities.dump
